...
from data_challenge.agoda_cancellation_estimator import AgodaCancellationEstimator
...
from data_challenge.agoda_cancellation_estimator_regression import AgodaCancellationEstimator1
...
def load_data(filename: str, with_lables = True):
    ...
    label_for_regression = labels.dt.date - features["booking_datetime"].dt.date
    ...
    return features, label_for_regression
...
if __name__ == '__main__':
    ...
    if for_test:
        ...
        exit()
        # Fit model over data
        estimator = AgodaCancellationEstimator1().fit(train_X.to_numpy(), train_y.to_numpy().astype(int))
