...
from data_challenge.Currency import RealTimeCurrencyConverter
from data_challenge.new_estimator import AgodaCancellationEstimator
...
def load_data(filename: str, with_lables = True):
    ...
    f = ["checkout_date", "booking_datetime", "checkin_date", "cancellation_policy_code",
              "original_payment_currency"]
...
def make_same_features(train_X: pd.DataFrame, test_X: pd.DataFrame):
    ...
    common_cols = [col for col in set(train_X.columns).intersection(test_X.columns)]
...
if __name__ == '__main__':
    ...
    if for_train:
        ...
        #     False)
        df, cancellation_labels = load_data(r"C:\שולחן העבודה\שנה ב\סמסטר ב\IML\Challenge\Data\combined_week6.csv")
        ...
        # print(f"Confusion Matrix : n{conf}")
        # print(f"Classification Report : n{clf_report}")
        print(f"F1: ", f1_score(test_y.to_numpy().astype(int), y_pred_lr))


    if not for_train:
        train_X, train_y = load_data("C:\שולחן העבודה\שנה ב\סמסטר ב\IML\Challenge\Data\TRAIN_SET.csv")
        print("****************************")
        test_x, test_y = load_data(
            r"C:\שולחן העבודה\שנה ב\סמסטר ב\IML\Challenge\Data\TEST_SET.csv", False)
