...
from Currency import RealTimeCurrencyConverter
from agoda_cancellation__estimator import AgodaCancellationEstimator
...
def f10(cancellation: str, full_price: float, night_price: float) -> (
        float, float):
...
def get_cancellation(features: pd.DataFrame):
    ...
    for index, row in features.iterrows():
        a, b = f10(row.cancellation_policy_code, row.original_selling_amount,
                   row.price_per_night)
        sum.append(a)
        no_show.append(b)
    return sum, no_show
...
def change_currency(features: pd.DataFrame):
    total = []
    for index, row in features.iterrows():
        total_amount = converter.convert(row.original_payment_currency, 'USD',
                                         row.original_selling_amount)
...
def load_data(filename: str, with_lables=True):
    ...
    features["duration"] = (features["checkout_date"] - features[
        "checkin_date"]).dt.days.astype(int)
    features['checkin_date_day_of_year'] = (
        features['checkin_date'].dt.dayofyear).astype(int)
    features["booking_hour"] = (
        pd.DatetimeIndex(features['booking_datetime']).hour).astype(int)
    selling_amount = change_currency(features)
    features["original_selling_amount"] = selling_amount
    features["price_per_night"] = (
            features["original_selling_amount"] / features["duration"])

    # fixing dummies features
    features = pd.get_dummies(features, prefix="hotel_star_rating_",
                              columns=["hotel_star_rating"])
    features = pd.get_dummies(features, prefix="accommadation_type_name_",
                              columns=["accommadation_type_name"])
    features = pd.get_dummies(features, prefix="charge_option_",
                              columns=["charge_option"])
    features = pd.get_dummies(features, prefix="customer_nationality_",
                              columns=["customer_nationality"])
    features = pd.get_dummies(features, prefix="no_of_adults_",
                              columns=["no_of_adults"])
    features = pd.get_dummies(features, prefix="no_of_children_",
                              columns=["no_of_children"])
    features = pd.get_dummies(features, prefix="original_payment_type_",
                              columns=["original_payment_type"])
    features = pd.get_dummies(features, prefix="original_payment_method_",
                              columns=["original_payment_method"])
    features = pd.get_dummies(features, prefix="hotel_city_code_",
                              columns=["hotel_city_code"])
    features[features["is_user_logged_in"] == "FALSE"] = 0
    features[features["is_user_logged_in"] == "TRUE"] = 1

    features["cancellation_sum"], features[
        "cancellation_no_show"] = get_cancellation(features)

    # removing old features
    f = ["checkout_date", "booking_datetime", "checkin_date",
         "cancellation_policy_code",
         "original_payment_currency"]
    ...
    # features = features[["cancellation_sum", "cancellation_no_show","price_per_night"]]
...
def make_same_features(train_X: pd.DataFrame, test_X: pd.DataFrame):
    ...
    common_cols = [col for col in
                   set(train_X.columns).intersection(test_X.columns)]
    new_test = test_X[common_cols]
    new_train = train_X[common_cols]
    return new_train, new_test


def print_stats(test_X, test_y):
    ...
    clf_report = classification_report(test_y.to_numpy().astype(int),
                                       y_pred_lr)
...
if __name__ == '__main__':
    ...
    if for_train:
        ...
        #     "C:\שולחן העבודה\שנה ב\סמסטר ב\IML\Challenge\Data\TRAIN_SET.csv")
        # print("****************************")
        # test_X, test_y = load_data(
        #     r"C:\שולחן העבודה\שנה ב\סמסטר ב\IML\Challenge\Data\TEST_SET.csv",
        #     False)
        df, cancellation_labels = load_data(r"C:\Users\omrib\PycharmProjects\IML\IML.HUJI\challenge\combined_week6.csv")
        # train_X, train_y, test_X, test_y = split_train_test(df, cancellation_labels, random_state = 0)
        print("****************************")
        train_X, test_X, train_y, test_y = sklearn.model_selection.train_test_split(
            df, cancellation_labels, random_state=0)
        ...
        # Fit model over data
        for k in range(40,55,5):
            estimator = AgodaCancellationEstimator(k).fit(train_X.to_numpy(),
        ...
        # evaluate_and_export(estimator, test_X, r"C:\Users\User\Documents\CSE_2\IML\dataChallenge\208881136_207029398_207543620_validation.csv")
            ...
            y1, y2, y3, y4, y5 = estimator.predict(test_X)
        # acc_lr = accuracy_score(test_y.to_numpy().astype(int), y_pred_lr)
        # conf = confusion_matrix(test_y.to_numpy().astype(int), y_pred_lr)
        ...
        # print(f"Classification Report : n{clf_report}")
            for j, y in enumerate([y1, y2, y3, y4, y5]):
                print(f"F1: for threshold = {30 + (j * 2)} ",
                      f1_score(test_y.to_numpy().astype(int), y))

        # print(f"F1: for threshold =", f1_score(test_y.to_numpy().astype(int), y_pred))

    if not for_train:
        train_X, train_y = load_data(r"C:\Users\omrib\PycharmProjects\IML\IML.HUJI\challenge\combined_week6.csv")
        print("****************************")
        test_x, test_y = load_data(
            r"C:\Users\omrib\PycharmProjects\IML\IML.HUJI\challenge\week_8_test_set.csv",
            False)
        ...
        # Fit model over data
        estimator = AgodaCancellationEstimator(64).fit(train_X.to_numpy(),
                                                     train_y.to_numpy().astype(
                                                         int))

        print("test_y", test_y)
        # print_stats(test_x, test_y)
        # Store model predictions over test set
        evaluate_and_export(estimator, test_x.to_numpy(),r"C:\Users\omrib\PycharmProjects\IML\IML.HUJI\challenge\208881136_207029398_207543620_week8.csv")
